...
int main(int argc , char *argv[])
{
  ...
  {    
    ...
    {
      // file exists
      //by opening the file with 'a' we write at the end of the file and we dont replace the stored data.
...
// Keep reading in integers (which are placed into "num") until end of file (EOF)
while ( result = fscanf ( fi, "%d%c", &num, &ws)) 
{
      ...
      //  printf("Row : %d\n",count );
        ...
        //and we want to update all the file so we overide it with "w"
        //and after that for the rest of the avgs we use the 'a' so that we dont override the 
        //new avgs that we instert.
        if(i<1)
        {
          AvgFile = fopen("AvgFromServerA","w");
          fprintf(AvgFile,"%d\n",AVG[i]);
          fclose(AvgFile);
        }
        else
        {
          AvgFile = fopen("AvgFromServerA","a");
          fprintf(AvgFile,"%d\n",AVG[i]);
          fclose(AvgFile);
...
while ( result = fscanf ( fi, "%d%c", &num, &ws)) 
{
        ...
        if(i<1)
        {
          FluFile = fopen("FluFromServerA","w");
           fprintf(FluFile,"%.2f\n",flu);
           fclose(FluFile);
        }
        else
